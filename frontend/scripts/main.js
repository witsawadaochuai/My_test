...
function processDashboardData(data) {
  ...
  data.forEach((item) => {
    ...
    if (!processedData.monthlyEngagements[month]) {
      processedData.monthlyEngagements[month] = {
        views: 0,
        comments: 0,
        likes: 0,
      };
    ...
    processedData.monthlyEngagements[month].comments +=
      item.engagement_comment || 0;
    ...
    if (!processedData.keywordByHour[hour])
      processedData.keywordByHour[hour] = {};
    keywords.forEach((k) => {
      if (!processedData.keywordByHour[hour][k])
        processedData.keywordByHour[hour][k] = 0;
  ...
  processedData.totalViews = data.reduce(
    (sum, d) => sum + (d.engagement_view || 0),
    0
  );
  processedData.totalLikes = data.reduce(
    (sum, d) => sum + (d.engagement_like || 0),
    0
  );
}
...
function createTimelineDataChart() {
  ...
  const messageCounts = hourLabels.map(
    (h) => processedData.hourlyCounts[h] || 0
  );
...
function createTimelineEngagementChart() {
  ...
  const views = labels.map(
    (m) => processedData.monthlyEngagements[m]?.views || 0
  );
  const likes = labels.map(
    (m) => processedData.monthlyEngagements[m]?.likes || 0
  );
  const comments = labels.map(
    (m) => processedData.monthlyEngagements[m]?.comments || 0
  );
...
function createKeywordChart() {
  ...
  const labels = sortedKeywords.map(
